...
class cc2_case(object):
    ...
    @property
    def prev_run_start_date(self):
        status_node = ET.parse(os.path.join(self.path, self._xml_config)).getroot().find('status')
        date = status_node.find('prev_run_start_date').text
        return None if date is None else datetime.strptime(date, date_fmt['in'])
    @prev_run_start_date.setter
    def prev_run_start_date(self, date):
        tree = ET.parse(os.path.join(self.path, self._xml_config))
        tree.find('status').find('prev_run_start_date').text = date.strftime(date_fmt['in'])
        tree.write(os.path.join(self.path, self._xml_config), xml_declaration=True)
    ...
    def to_xml(self):
        ...
        ET.SubElement(status_node, 'prev_run_start_date')
...
while (( ${{#}} > 0 )); do
    ...
    def _submit_archive_cmd(self):
        ...
        if self.prev_run_start_date is not None and self._run_start_date > self.start_date:
            _submit_archive_chunck(self.prev_run_start_date, self._run_start_date)

        # Archive last chunck
        if self._run_end_date >= self.end_date:
            _submit_archive_chunck(self._run_start_date, self.end_date)

        self.prev_run_start_date = self._run_start_date
